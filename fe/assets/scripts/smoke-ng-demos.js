/*
  Build every Angular demo and load it in a real browser.

  Compiling proves very little: a demo can build cleanly and still throw on its
  first render - that is exactly how the React demos behaved. This installs the
  dependencies once, then for each demo builds it, serves its dist/ and loads it
  in headless Edge, reporting build failures, uncaught exceptions and pages that
  render nothing.

  Usage
    node scripts/smoke-ng-demos.js                    # every Angular demo
    node scripts/smoke-ng-demos.js --group grid       # one widget folder
    node scripts/smoke-ng-demos.js --limit 40         # a sample
    node scripts/smoke-ng-demos.js --jobs 4 --work C:/jqxng

  Writes demos/smoke-ng-report.json. Exit code is non-zero if anything failed.
  Needs Node.js, npm and an installed Edge or Chrome.
*/
'use strict';
...
// A short path: these dependency trees go past the Windows path limit anywhere deeper.
const WORK = path.resolve(arg('work', 'C:/jqxng'));
...
if (LIMIT) {
    // spread the sample across widgets rather than taking the first N of one
...
function recorder(slot) {
    ...
    '        if (sent) { return; }\n' +
    '        sent = true;\n' +
    '        var host = document.querySelector("app-root");\n' +
    '        var inner = host ? host.innerHTML : "";\n' +
    '        var payload = {\n' +
    '            errors: errs.slice(0, 3),\n' +
    '            rendered: inner.replace(/\\s|Loading\\.\\.\\./g, "").length,\n' +
...
const MIME = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.mjs': 'text/javascript; charset=utf-8', '.css': 'text/css; charset=utf-8', '.json': 'application/json; charset=utf-8', '.txt': 'text/plain; charset=utf-8', '.xml': 'application/xml; charset=utf-8', '.csv': 'text/csv; charset=utf-8', '.tsv': 'text/tab-separated-values; charset=utf-8', '.svg': 'image/svg+xml', '.png': 'image/png', '.jpg': 'image/jpeg', '.gif': 'image/gif', '.ico': 'image/x-icon', '.woff': 'font/woff', '.woff2': 'font/woff2', '.ttf': 'font/ttf', '.eot': 'application/vnd.ms-fontobject', '.map': 'application/json' };

// Every worker gets its OWN server on its OWN port, serving its dist at the root.
//
// Serving the workers under a shared /w<slot>/ prefix looks tidier but quietly
// breaks the demos: many of them read their data with a relative '../assets/x',
// which from /w0/ escapes the prefix to /assets/x and 404s. At the root the URLs
// resolve exactly as they do in a real deployment, and <base href="/"> is correct
// as built, so nothing has to be rewritten.
//
...
function serverFor(slot) {
    return http.createServer((req, res) => {
        ...
        const distRoot = path.join(WORK, 'w' + slot, 'dist');
        // The Angular builder writes into dist/<projectName>/[browser], and the demos do
        // not agree on the project name - most say "demo", some say "app". Rather than
        // guess, find the index.html the build actually produced.
        let base = distRoot;
        (function findIndex(dir, depth) {
            if (depth > 3) { return false; }
            if (fs.existsSync(path.join(dir, 'index.html'))) { base = dir; return true; }
            let entries = [];
            try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch (e) { return false; }
            for (const e of entries) {
                if (e.isDirectory() && findIndex(path.join(dir, e.name), depth + 1)) { return true; }
            }
            return false;
        })(distRoot, 0);
        ...
        fs.stat(file, (err, st) => {
            ...
            if (ext === '.html') {
                const html = fs.readFileSync(file, 'utf8').replace(/<head[^>]*>/i, mm => mm + recorder(slot));
                return res.end(html);
            }
...
async function main(basePort, servers) {
    ...
    if (!fs.existsSync(path.join(tpl, 'node_modules'))) {
        console.log('installing the shared dependency tree once (a minute or so)...');
        copyDir(demos[0].dir, tpl);
        const r = await run('npm', ['install', '--no-audit', '--no-fund', '--loglevel=error'], tpl, 20 * 60 * 1000);
        if (!fs.existsSync(path.join(tpl, 'node_modules'))) { console.error('install failed:\n' + r.out.slice(-1500)); process.exit(2); }
        console.log('done\n');
    }
    const NM = path.join(tpl, 'node_modules');

    console.log(`${demos.length} Angular demos, ${JOBS} in parallel\n`);
    ...
    async function worker(slot) {
        ...
        while (next < demos.length) {
            ...
            const build = await run('npx', ['--no-install', 'ng', 'build', '--configuration', 'production'], w, 6 * 60 * 1000);
            const built = /bundle generation complete|Initial total|Build at|Output location/i.test(build.out);
            if (process.env.JQX_NG_DEBUG) { console.log('    [debug] build output tail: ' + build.out.replace(/s+/g,' ').slice(-200)); }
            if (!built) {
                const lines = build.out.split('\n').filter(l => /error|ERROR|✘/i.test(l)).slice(0, 3);
                r.errors.push('build failed: ' + (lines.join(' | ').replace(/\s+/g, ' ').slice(0, 260) || 'no success marker'));
            } else {
                ...
                if (!page) r.errors.push('the page never reported back within ' + Math.round(PAGE_MS / 1000) + 's');
                    ...
                    // Only what Angular rendered counts: the build inlines
                    // jqx.base.css, and that stylesheet is full of .jqx- selectors.
                    if (!page.rendered) r.errors.push('app-root is empty: the component did not render');
                }
            }
            if (process.env.JQX_NG_DEBUG) {
                console.log('    [debug] ' + d.id + ' built=' + built + ' errors=' + JSON.stringify(r.errors) + ' widget=' + r.widget + ' note=' + r.note);
            }
            results.push(r);
            done++;
    ...
    const reportName = partial ? 'smoke-ng-report.partial.json' : 'smoke-ng-report.json';
