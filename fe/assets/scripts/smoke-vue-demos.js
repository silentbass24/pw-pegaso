/*
  Build every Vue demo and load it in a real browser.

  Same reasoning as the Angular and React sweeps: building proves the bundle
  compiles, not that the page works. This installs the dependencies once, then
  for each demo runs its webpack build, serves the demo folder and loads its
  index.htm in headless Edge, reporting build failures, uncaught exceptions and
  pages where #app stays empty.

  Usage
    node scripts/smoke-vue-demos.js                   # every Vue demo
    node scripts/smoke-vue-demos.js --group grid      # one widget folder
    node scripts/smoke-vue-demos.js --limit 40        # a sample
    node scripts/smoke-vue-demos.js --list ids.txt    # exactly these demos
    node scripts/smoke-vue-demos.js --jobs 4 --work C:/jqxvue

  Writes demos/smoke-vue-report.json. Exit code is non-zero if anything failed.
*/
'use strict';
...
const WORK = path.resolve(arg('work', 'C:/jqxvue'));
...
(function () {
    ...
    function send() {
        if (sent) { return; }
        sent = true;
        // Vue 2 REPLACES the mount element with the component's root, so once the app
        // is up there is no #app to look at. Measure the body instead, minus the
        // script tags that were there before Vue ran.
        var inner = document.body ? document.body.innerHTML.replace(/<script[\\s\\S]*?<\\/script>/gi, '') : '';
        var payload = {
...
function serverFor(slot) {
    return http.createServer((req, res) => {
        ...
        const rest = (urlPath === '/' || urlPath === '') ? '/index.htm' : urlPath;
        const base = path.join(WORK, 'w' + slot);
...
async function main(basePort, servers) {
    ...
    console.log(`${demos.length} Vue demos, ${JOBS} in parallel\n`);
    ...
    async function worker(slot) {
        ...
        while (next < demos.length) {
            ...
            const build = await run('npx', ['--no-install', 'webpack', '--config', 'webpack.config.js'], w, 6 * 60 * 1000);
            const bundle = path.join(w, 'dist', 'main.bundle.js');
            if (!fs.existsSync(bundle)) {
                ...
                if (!page) r.errors.push('the page never reported back within ' + Math.round(PAGE_MS / 1000) + 's');
                    ...
                    // Only what Vue rendered counts: the page also links jqx.base.css,
                    // whose text is full of .jqx- selectors.
                    if (!page.rendered) r.errors.push('the page is empty: the component did not render');
                }
            }
            results.push(r);
            done++;
    ...
    const reportName = partial ? 'smoke-vue-report.partial.json' : 'smoke-vue-report.json';
