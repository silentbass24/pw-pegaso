...
@Component({
    selector: 'app-root',
    imports: [jqxProgressBarModule, jqxRadioButtonModule, jqxInputModule, jqxButtonModule, jqxCheckBoxModule],
    standalone: true,
...
export class AppComponent {
    ...
    getValueInput(): number {
        ...
        if (!this.ValueInputElement) { return; }
    ...
    onCheckBox(event: any): void {
        // the widget raises this while initialising, before @ViewChild is populated
        if (!this.myHorizontalProgressBar || !this.myVerticalProgressBar) { return; }
        let value = this.getValueInput();
