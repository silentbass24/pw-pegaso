...
export class AppComponent implements AfterViewInit {
    ...
    myGridOnRowClick(event: any): void | boolean {
        ...
        if (!this.myGrid || !this.myMenu) { return; }
    ...
    myMenuOnItemClick(event: any): void {
        // the widget raises this while initialising, before @ViewChild is populated
        if (!this.myGrid || !this.myWindow || !this.firstName || !this.lastName || !this.product || !this.quantity || !this.price) { return; }
        let args = event.args;
    ...
    SaveBtnOnClick(): void {
        if (this.editrow >= 0) {
            let row = {
                ...
                quantity: parseInt(String(this.quantity.decimal())), price: parseFloat(String(this.price.decimal()))
            };
