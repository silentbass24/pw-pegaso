...
export class AppComponent implements AfterViewInit {
    ...
    myGridOnCellSelect(event: any): void {
        // the widget raises this while initialising, before @ViewChild is populated
        if (!this.myGrid || !this.selectedCell) { return; }
        let column = this.myGrid.getcolumn(event.args.datafield);
        // the grid raises this once as it initialises, before the columns are addressable
        if (!column) { return; }
        let columnheader = column.text;
    ...
    myGridOnCellUnselect(event: any): void {
        // the widget raises this while initialising, before @ViewChild is populated
        if (!this.myGrid || !this.unselectedCell) { return; }
        let column = this.myGrid.getcolumn(event.args.datafield);
        if (!column) { return; }
        let columnheader = column.text;
