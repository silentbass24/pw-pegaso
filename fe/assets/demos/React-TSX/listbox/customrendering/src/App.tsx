...
const App = () => {
    ...
    const onSelect = useCallback((event: any): void => {
        updatePanel(event.args.index);
    }, []);

    const updatePanel = useCallback((index: number): void => {
        ...
        renderInto(container, contentPanel.current!);
    }, []);
    ...
    const renderer = (index, label, value): any => {
                const datarecord = getData()[index!];
    ...
    return (
        <JqxSplitter theme={'material-purple'} width={600} height={600} panels={[{ size: '40%' }]}>
            <div style={{ overflow: 'hidden' }}>
                <JqxListBox theme={'material-purple'} ref={myListBox} style={{ border: 'none' }} onSelect={onSelect}
                    width={'100%'} height={'100%'} selectedIndex={0} source={source} itemHeight={70}
                    displayMember={'firstname'} valueMember={'notes'} renderer={renderer} />
            </div>
