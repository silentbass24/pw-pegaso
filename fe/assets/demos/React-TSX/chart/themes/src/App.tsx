...
const swatches = (index: number, marginTop: string): string => {
    const colors = schemes[index].colors;
    let html = '';
    for (const color of colors) {
        html += '<div style="' + marginTop + 'float: left; border-radius: 100%; width: 25px; height: 25px; border: none; background:' + color + '; margin-left: 4px;"></div>';
    }
    return html;
};
...
const selectionTemplate = (element: object, index: number): string => {
    ...
    return swatches(index, 'margin-top: 2px; ');
};

const App = () => {
    const [colorScheme, setColorScheme] = useState<IChartProps['colorScheme']>('scheme01');
    const [selectedIndex, setSelectedIndex] = useState<IDropDownListProps['selectedIndex']>(0);

    const padding = useMemo(() => ({ left: 5, top: 5, right: 5, bottom: 5 }), []);
    const titlePadding = useMemo(() => ({ left: 90, top: 0, right: 0, bottom: 10 }), []);
    ...
    const seriesGroups = useMemo<IChartProps['seriesGroups']>(() => [
        {
    ...
    const myDropDownListOnChange = useCallback((event: any): void => {
    ...
    return (
        <div>
            <JqxChart style={{ width: '850px', height: '500px' }}
                ...
                seriesGroups={seriesGroups} colorScheme={colorScheme} />

            <br />
            Themes:

            <JqxDropDownList theme={'material-purple'} onChange={myDropDownListOnChange}
                width={180} height={30} selectedIndex={selectedIndex} itemHeight={35}
