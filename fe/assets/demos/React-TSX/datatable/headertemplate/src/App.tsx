...
const App = () => {
    ...
    const updatingSelection = useRef<boolean>(undefined as any);
    const updatingSelectionFromDataTable = useRef<boolean>(undefined as any);

    const tableOnRowSelect = useCallback((): void => {
        updatingSelectionFromDataTable.current = true;
        if (!updatingSelection.current) {
            myCheckBox.current!.setOptions({ checked: allRowsSelected() });
        }
        updatingSelectionFromDataTable.current = false;
    }, []);

    const tableOnRowUnselect = useCallback((): void => {
        updatingSelectionFromDataTable.current = true;
        if (!updatingSelection.current) {
            myCheckBox.current!.setOptions({ checked: allRowsSelected() });
        }
    ...
    const allRowsSelected = useCallback(() => {
    ...
    const generateData = useCallback((): any[] => {
        const data = [];
    ...
    useEffect(() => {
        ...
        const onChange = (event: any) => {
            if (!updatingSelectionFromDataTable.current) {
                const args = event.args;
                const rows = myDataTable.current!.getRows();
                updatingSelection.current = true;
                if (args.checked) {
                    for (let i = 0; i < rows.length; i++) {
                        myDataTable.current!.selectRow(i);
                    }
                }
                else {
        ...
        renderInto(
            <JqxCheckBox theme={'material-purple'} ref={myCheckBox} onChange={onChange}/>,
    ...
    return (
        <JqxDataTable theme={'material-purple'} ref={myDataTable}
            onRowSelect={tableOnRowSelect} onRowUnselect={tableOnRowUnselect}
            // @ts-ignore
