...
2.Examples
  ...
  Quickest way to see everything: open index.htm, or double-click start-demos.cmd
  (Windows) / run "sh start-demos.sh" (macOS, Linux). That starts a tiny local web
  server (needs only Node.js, nothing is installed) and opens the Demo Browser at
  demos/index.htm - all 3,600 demos for JavaScript, TypeScript, Angular, React and
  Vue with live preview, search, theme switching, source view and StackBlitz export.
  demos/index.htm also works when opened straight from disk, but demos that load
  data with Ajax need the web server.
  ...
  The API reference pages are in the /demos/Javascript & JQuery/documentation directory
  The php & mysql integration demos are located in the demos/PHP directory.
  All php integration samples use and require the Northwind Database(/demos/PHP/Northwind.MySQL5.sql).
  Any examples that use Ajax need to be on a Web Server/LocalHost in order to work correctly.
  Angular examples are located in the /demos/Angular directory (one Angular CLI project per demo: npm install, npm start).
  Vue examples are located in the /demos/Vue.js directory (one project per demo: npm install, npm start, then serve index.htm).
  React examples are located in the /demos/React-TSX directory (one Vite project per demo: npm install, npm run dev).
  TypeScript examples are located in the /demos/Typescript directory (pre-compiled, run without a build).
  Full sample applications are in the /demos/Showcase directory.
  ...
  Legacy AngularJS 1.x examples are in /demos/Angular1.x.
  Web Components / Custom Elements are a separate product: https://www.htmlelements.com/

3.Documentation

  Browse the documentation and examples online on our website: https://www.jqwidgets.com/
   
3a.Maintenance scripts (Node.js, no dependencies)

  node scripts/serve-demos.js          Serve this folder and open the Demo Browser.
  node scripts/build-demo-catalog.js   Rebuild demos/catalog.js after adding or renaming demos.
  node scripts/align-demo-versions.js  Put every Angular/React/Vue demo on this SDK's jQWidgets
                                       version (--dry-run to preview).
  node scripts/verify-demo-builds.js   npm install + build a sample of the demos; non-zero exit
                                       on failure, so CI can gate on it.
  node scripts/fix-angular-imports.js  Add the modules/symbols an Angular demo uses but does not
                                       import (--dry-run to preview).
  node scripts/fix-react19-render.js   Move React demos off the ReactDOM.render that React 19
                                       removed (--dry-run to preview).
  node scripts/smoke-js-demos.js        Load every JavaScript demo in a headless browser and
                                       report the ones that throw (about 11 minutes).
  node scripts/fix-js-logo-injection.js Repair demos where the logo block was spliced into a
                                       JavaScript string (--dry-run to preview).
  node scripts/smoke-ng-demos.js       Build and browser-test every Angular demo (~45 min).
  node scripts/smoke-vue-demos.js      Build and browser-test every Vue demo (~60 min).
  sh scripts/smoke-react-demos.sh <list> <workdir>
                                       Load React demos in a headless browser and fail on any
                                       that do not render.
