$(document).ready(function () {
    ...
    (function init() {
        ...
        $.ajax({
            url: `${API_BASE}/api/anamnesi-familiare/${id}`,
            method: 'GET',
            dataType: 'json',
            cache: false
        }).done(function (a) {
            esiste = true;
            $('#malattie_ereditarie').val(a.malattie_ereditarie || '');
            $('#note_ambientali').val(a.note_ambientali || '');
    ...
    $("#nuova-anamnesi-familiare").on("submit", function (e) {
        e.preventDefault();
        ...
        const datiAnagrafica = {
            malattie_ereditarie: $("#malattie_ereditarie").val(),
            note_ambientali: $("#note_ambientali").val(),
            id_paziente: id
        };
        ...
        if (esiste) {
            $.ajax({
                url: `${API_BASE_URL}/api/aggiorna-anamnesi-familiare/${id}`,
                method: 'PUT',
                contentType: 'application/json',
                data: JSON.stringify(datiAnagraficaAgg)
            }).done(function () {
                alert("Anamnesi familiare aggiornata correttamente");
                window.location.replace(`nuova-anamnesi-familiare.html?id=${id}`);
            }).fail(function (xhr) {
                $btn.prop("disabled", false).text(originalBtnText);
                alert("Errore aggiornamento anamnesi familiare: " + (xhr.responseText || xhr.status));
            });
        } else {
            $.ajax({
                url: `${API_BASE_URL}/api/nuova-anamnesi-familiare`,
                method: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(datiAnagrafica)
            }).done(function () {
                alert("Anamnesi familiare creata correttamente");
                window.location.replace(`nuova-anamnesi-familiare.html?id=${id}`);
            }).fail(function (xhr) {
                $btn.prop("disabled", false).text(originalBtnText);
                alert("Errore creazione anamnesi familiare: " + (xhr.responseText || xhr.status));
            });
        }
    });
});